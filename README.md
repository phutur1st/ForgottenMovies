...
| `UNSUBSCRIBE_SECRET_KEY` | Optional: 256-bit secret for encrypting email addresses in unsubscribe links. Generate with `python -c "import secrets; print(secrets.token_hex(32))"`. Leave unset to disable self-service unsubscribe. |
...
When enabled, reminder emails include an unsubscribe link that lets users manage their subscription without admin intervention. The feature uses XOR encryption with a secret key so links can't be forged or decrypted without the secret.
...
If you're exposing the unsubscribe endpoints publicly via nginx/Swag, add rate limiting to prevent DoS attacks.
...
limit_req_zone $binary_remote_addr zone=unsubscribe_limit:10m rate=10r/m;
